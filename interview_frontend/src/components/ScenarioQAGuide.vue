<template>
  <el-dialog
    ...
    :show-close="false"
    class="scenario-qa-guide-dialog"
    center
  >
    <!-- 指引内容 -->
    <div class="guide-content">
      ...
      <div class="main-info">
        ...
        <div class="info-text">
          <h3 class="title">场景问答</h3>
          <p class="description">结合项目经历，真实回答</p>
        </div>
      </div>
...
// 语音指引文本
const guideText = '接下来是场景问答环节。AI面试官将根据您提交的简历内容进行提问。请结合您的实际项目经历和工作经验来回答问题。请保持真实，详细描述您在项目中的具体职责和贡献。现在开始场景问答。'

// 停止语音播放
const stopVoice = () => {
  console.log('🔇 场景问答指引：强制停止语音播放')
...
const playVoice = async () => {
  ...
  try {
    ...
    if (audioElementRef.value) {
      ...
      console.log('🎵 场景问答指引语音开始播放')
    }
  } catch (error) {
    console.error('🎵 播放场景问答指引语音失败:', error)
    console.error('语音播放失败，将自动开始场景问答')
    isPlayingVoice.value = false
    voiceCompleted.value = true
    // 即使语音播放失败，也要在短暂延迟后开始问答
    setTimeout(() => {
      handleAutoStart()
    }, 1500)
  }
...
const handleAudioError = (error) => {
  ...
  if (isPlayingVoice.value) {
    console.error('🎵 场景问答指引音频播放错误:', error)
    console.error('语音播放出现错误，将自动开始场景问答')
    // 出错时也要自动开始问答
    setTimeout(() => {
      handleAutoStart()
    }, 1500)
...
<style scoped>
.scenario-qa-guide-dialog {
  --el-dialog-border-radius: 16px;
}

.scenario-qa-guide-dialog :deep(.el-dialog) {
  border-radius: 16px;
  box-shadow: 0 20px 40px rgba(0, 0, 0, 0.15);
  backdrop-filter: blur(10px);
}

.scenario-qa-guide-dialog :deep(.el-dialog__header) {
...
.scenario-qa-guide-dialog :deep(.el-dialog__title) {
  font-size: 20px;
  font-weight: 700;
  color: white;
}

.scenario-qa-guide-dialog :deep(.el-dialog__body) {
